...
void scnInit(struct ADAPTER *prAdapter)
{
	...
	prScanInfo->ucScnChIdleTimeAbnormalCnt = 0;
	prScanInfo->ucScnChIdleTimeAbnormalResetCnt = 0;
#endif
...
{
	uint8_t ucRxRCPI, ucChannelDiff;
	...
	/* 5G Band */
		...
		if (ucIeHtChannelNum > ucHwChannelNum)
			ucChannelDiff = ucIeHtChannelNum - ucHwChannelNum;
		else
			ucChannelDiff = ucHwChannelNum - ucIeHtChannelNum;
		...
		if (ucIeHtChannelNum >= 1 && ucIeHtChannelNum < 200) {
			...
			/* Default trust HW channel */
			prBssDesc->ucChannelNum = ucHwChannelNum;

			/* Receive Beacon/ProbeResp frame
			 * from 5G adjacent channel.
			 */
			if ((ucIeHtChannelNum != ucHwChannelNum)
				&& (ucChannelDiff <= CHNL_SPAN_20))
				/* trust channel information brought by IE */
				prBssDesc->ucChannelNum = ucIeHtChannelNum;
			else
				DBGLOG(SCN, INFO,
					"[DEBUG]Trust HW channel=%d, instead of HT channel=%d",
					ucHwChannelNum, ucIeHtChannelNum);
		} else {
			/* Always update RCPI */
			prBssDesc->ucRCPI = ucRxRCPI;
			prBssDesc->ucChannelNum = ucHwChannelNum;
...
bool isH3cFtIeExistFunc(uint8_t *h3cftie)
{
	uint32_t oui = 0, vendortype = 0;
	WLAN_GET_FIELD_BE24(h3cftie, &oui);
	WLAN_GET_FIELD_BE24(&h3cftie[3], &vendortype);
	if (oui == VENDOR_OUI_H3C
		&& vendortype == VENDOR_H3C_OUI_TYPE
		&& h3cftie[6] == ELEM_ID_MOBILITY_DOMAIN
		&& h3cftie[7] == 0x03) {
		return TRUE;
	}
	return FALSE;
...
uint32_t addH3cAkmSuiteFunc(uint8_t *s) {
	uint32_t akm = 0, res = 0;
	WLAN_GET_FIELD_32(s, &akm);
	if (akm == RSN_AKM_SUITE_802_1X || akm == RSN_AKM_SUITE_802_1X_SHA256) {
		res = RSN_AKM_SUITE_FT_802_1X;
	}
	if (akm == RSN_AKM_SUITE_PSK || akm == RSN_AKM_SUITE_PSK_SHA256) {
		res = RSN_AKM_SUITE_FT_PSK;
	}
	return res;
}

uint16_t generateH3cRsnIeFunc(uint8_t *buf, uint8_t *rsn_ie, uint16_t rsn_ie_len)
{
	uint8_t *pos = buf;
	uint16_t count = 0;
	uint32_t add_akm = 0;
	...
	*pos++ = ELEM_ID_RSN;
	*pos++ = 4 + rsn_ie_len - ELEM_HDR_LEN;
	rsn_ie += ELEM_HDR_LEN;
	rsn_ie_len -= ELEM_HDR_LEN;

	/* 1.version ie */
	kalMemCopy(pos, rsn_ie, 2);
	pos += 2;
	rsn_ie += 2;
	rsn_ie_len -= 2;

	/* 2.group data cipher suite(0 or 4) */
	if (rsn_ie_len >= 4) {
		kalMemCopy(pos, rsn_ie, 4);
		pos += 4;
		rsn_ie += 4;
		rsn_ie_len -= 4;
	}

	/* 3.Pairwise Cipher Suite[count(0 or 2), list(0 or 4xm)] */
	if (rsn_ie_len >= 2) {
		WLAN_GET_FIELD_16(rsn_ie, &count);
		kalMemCopy(pos, rsn_ie, 2 + count * 4);
		pos += (2 + count * 4);
		rsn_ie += (2 + count * 4);
		rsn_ie_len -= (2 + count * 4);
	}

	/* 4.AKM Suite[count(0 or 2), list(0 or 4xn)] */
	if (rsn_ie_len >= 2) {
		WLAN_GET_FIELD_16(rsn_ie, &count);
		add_akm = addH3cAkmSuiteFunc(rsn_ie + 2);
		if (count == 1 && add_akm != 0) {
			WLAN_SET_FIELD_16(pos, count + 1);
			pos += 2;
			rsn_ie += 2;
			rsn_ie_len -= 2;
			kalMemCopy(pos, rsn_ie, 4);
			WLAN_SET_FIELD_32(pos + 4, add_akm);
			pos += 8;
			rsn_ie += 4;
			rsn_ie_len -= 4;
		} else {
	...
	/* 6.PMKID[Count(0 or 2), list(0 or 16xs)] */
	if (rsn_ie_len >= 2) {
		WLAN_GET_FIELD_16(rsn_ie, &count);
		kalMemCopy(pos, rsn_ie, 2 + count * IW_PMKID_LEN);
		pos += (2 + count * IW_PMKID_LEN);
		rsn_ie += (2 + count * IW_PMKID_LEN);
		rsn_ie_len -= (2 + count * IW_PMKID_LEN);
	}

	/* 7.Group Management Cipher Suite[0 or 4]*/
	if (rsn_ie_len >= 4) {
		kalMemCopy(pos, rsn_ie, 4);
		pos += 4;
		rsn_ie += 4;
...
bool recreateH3cPucIeFunc(uint8_t *buf, uint8_t *ie, uint16_t ie_len)
{
	uint8_t *h3cftie = NULL, *rsnie = NULL, *ies = ie;
	uint16_t h3cftie_len = 0, rsnie_len = 0, ies_len = ie_len, buf_len = 0, u2Offset = 0;
	bool isH3cFtIeExist = FALSE, isRsnIeExist = FALSE, isMdIeExist = FALSE, isRecreateBuf = FALSE;

	if (!ie || ie_len == 0) {
		return FALSE;
	}

	h3cftie = kalMemAlloc(VENDOR_MDIE_LEN, VIR_MEM_TYPE);
	if (h3cftie == NULL) {
		goto exit;
	}
	kalMemZero(h3cftie, VENDOR_MDIE_LEN);

	rsnie = kalMemAlloc(ELEM_HDR_LEN + VENDOR_RSNIE_MAX_LEN - 4, VIR_MEM_TYPE);
	if (rsnie == NULL) {
		goto exit;
	}
	kalMemZero(rsnie, ELEM_HDR_LEN + VENDOR_RSNIE_MAX_LEN - 4);

	IE_FOR_EACH(ie, ie_len, u2Offset) {
		if (IE_ID(ie) != ELEM_ID_SSID && IE_LEN(ie) == 0) {
			continue;
		}
		switch (IE_ID(ie)) {
			case ELEM_ID_VENDOR:
				if (IE_LEN(ie) == VENDOR_OUI_H3C_LEN && isH3cFtIeExistFunc(&IE_ID_EXT(ie))) {
					h3cftie_len = VENDOR_MDIE_LEN;
					kalMemCopy(h3cftie,
						ie + ELEM_HDR_LEN + ELEM_MIN_LEN_WFA_OUI_TYPE_SUBTYPE, h3cftie_len);
					isH3cFtIeExist = TRUE;
				}
				break;
			case ELEM_ID_RSN:
				rsnie_len = IE_SIZE(ie);
				if (rsnie_len <= 0 || rsnie_len > ELEM_HDR_LEN + VENDOR_RSNIE_MAX_LEN - 4) {
					goto exit;
				}
				kalMemCopy(rsnie, ie, rsnie_len);
				isRsnIeExist = TRUE;
				break;
	...
	if (isMdIeExist || !isH3cFtIeExist || !isRsnIeExist) {
		goto exit;
	}

	kalMemCopy(buf, h3cftie, h3cftie_len);
	buf_len += h3cftie_len;

	if (generateH3cRsnIeFunc(buf + buf_len, rsnie, rsnie_len) != 4 + rsnie_len) {
		goto exit;
	} else {
		buf_len += (4 + rsnie_len);
	}

	IE_FOR_EACH(ies, ies_len, u2Offset) {
		if (IE_ID(ies) != ELEM_ID_RSN) {
			kalMemCopy(buf + buf_len, ies, IE_SIZE(ies));
			buf_len += IE_SIZE(ies);
		}
	}
...
exit:
	if (h3cftie) {
		kalMemFree(h3cftie, VIR_MEM_TYPE, VENDOR_MDIE_LEN);
		h3cftie = NULL;
	}
	if (rsnie) {
		kalMemFree(rsnie, VIR_MEM_TYPE, ELEM_HDR_LEN + VENDOR_RSNIE_MAX_LEN - 4);
		rsnie = NULL;
	}

	return isRecreateBuf;
}
				  ...
				  struct SW_RFB *prSwRfb)
{
...
#ifdef OPLUS_FEATURE_WIFI_VENDOR_FT
	uint8_t *recreate_buf = NULL;
	bool is_recreate_buf = FALSE;
	uint16_t max_ie_len = 0;
#endif /* OPLUS_FEATURE_WIFI_VENDOR_FT */
...
#ifdef OPLUS_FEATURE_WIFI_VENDOR_FT
	max_ie_len = 5 + 4 + u2IELength;
	recreate_buf = kalMemAlloc(max_ie_len, VIR_MEM_TYPE);
	if (!recreate_buf) {
		is_recreate_buf = FALSE;
	} else {
		kalMemZero(recreate_buf, max_ie_len);
		is_recreate_buf = recreateH3cPucIeFunc(recreate_buf, pucIE, u2IELength);
	}

	if (is_recreate_buf) {
		kalMemCopy(prWlanBeaconFrame->aucInfoElem, recreate_buf, max_ie_len);
		prSwRfb->u2PacketLen = max_ie_len + prSwRfb->u2HeaderLen +
			(uint16_t) OFFSET_OF(struct WLAN_BEACON_FRAME_BODY, aucInfoElem[0]);
		pucIE = prWlanBeaconFrame->aucInfoElem;
		u2IELength = (prSwRfb->u2PacketLen - prSwRfb->u2HeaderLen) -
			(uint16_t) OFFSET_OF(struct WLAN_BEACON_FRAME_BODY, aucInfoElem[0]);
	...
	if (recreate_buf) {
		kalMemFree(recreate_buf, VIR_MEM_TYPE, max_ie_len);
		recreate_buf = NULL;
	}
#endif /* OPLUS_FEATURE_WIFI_VENDOR_FT */

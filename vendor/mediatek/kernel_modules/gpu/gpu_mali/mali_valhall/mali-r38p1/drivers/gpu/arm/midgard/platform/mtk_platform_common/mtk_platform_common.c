...
int mtk_common_device_init(struct kbase_device *kbdev)
{
	...
	if (IS_ERR_OR_NULL(kbdev->mali_kthread_worker))
		dev_err(kbdev->dev, "Failed to create mali worker thread");
	else
		sched_set_fifo(kbdev->mali_kthread_worker->task);
#endif
...
#if IS_ENABLED(CONFIG_MALI_MTK_KTHREAD_ENHANCE)
	if (!IS_ERR_OR_NULL(kbdev->mali_kthread_worker))
		kthread_destroy_worker(kbdev->mali_kthread_worker);
#endif

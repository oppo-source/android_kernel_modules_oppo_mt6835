...
extern int mtk_drm_setbacklight_without_lock_v0(struct drm_crtc *crtc, unsigned int level, unsigned int panel_ext_param, unsigned int cfg_flag);
...
char display_brightness_time[32] = "\0";
EXPORT_SYMBOL(display_brightness_time);
...
int panel_serial_number_read(struct drm_crtc *crtc, char cmd, int num)
{
	...
	char NT37703A[] = {0xF0, 0x55, 0xAA, 0x52, 0x08, 0x01};
	char NT37703A_default[] = {0xF0, 0x55, 0xAA, 0x52, 0x08, 0x00};
	char ILI97601A[] = {0xFF, 0x5A, 0xA5, 0x1D};
	char ILI97601A_default[] = {0xFF, 0x5A, 0xA5, 0x00};
	char *panel_name;
	int count = 10;
	...
	if (!strcmp(panel_name, "AC068_P_3_A0001_fhd_cmd")) {
		...
		ddic_dsi_send_cmd(3, read_samsung_page0);
	} else if (!strcmp(panel_name, "ae045_p_1_a0017_fhd_vdo")) {
		pr_info("ae045_p_1_a0017_fhd_vdo set page\n");
		ddic_dsi_send_cmd(3, read_samsung_page0);
	} else if (!strcmp(panel_name, "ac312_p_3_a0025_dsi_vdo")) {
		pr_info("ac312_p_3_a0025_dsi_vdo set page\n");
	} else if (!strcmp(panel_name, "panel_ae112_p_3_a0025_dsi_vdo")) {
		pr_info("panel_ae112_p_3_a0025_dsi_vdo set page\n");
	} else if (!strcmp(panel_name, "ac388_p_7_a0025_dsi_vdo")) {
		pr_info("panel_ac388_p_7_a0025_dsi_vdo set page1\n");
		ddic_dsi_send_cmd(6, NT37703A);
	} else if (!strcmp(panel_name, "ac388_p_b_a0029_dsi_vdo")) {
		pr_info("panel_ac388_p_b_a0029_dsi_vdo set page1d\n");
	...
	while (count > 0) {
		...
		} else if (!strcmp(panel_name, "ac176_p_1_a0017_fhd_vdo") || !strcmp(panel_name, "ae045_p_1_a0017_fhd_vdo")) {
		...
		|| !strcmp(panel_name, "oplus21861_boe_nt37701a_fhd_dsi_cmd")) {
			...
			|| (!strcmp(panel_name, "panel_ae211_p_3_a0027_dsi_vdo"))) {
			panel_serial_info.reg_index = 0;
			panel_serial_info.year      = ((para[panel_serial_info.reg_index] & 0xF0) >> 4) + 9;
		} else if (!strcmp(panel_name, "panel_ae211_p_3_a0025_dsi_vdo")) {
			panel_serial_info.reg_index = 0;
	...
	if (!strcmp(panel_name, "ac162_p_3_a0004_cmd_panel")) {
		...
		pr_info("ac318_p_7_a0025_dsi_vdo set page default\n");
		ddic_dsi_send_cmd(6, NT37703A_default);
	} else if (!strcmp(panel_name, "ac388_p_b_a0029_dsi_vdo")) {
		pr_info("ac388_p_b_a0029_dsi_vdo set default\n");
		ddic_dsi_send_cmd(4, ILI97601A_default);
	}
...
static ssize_t oplus_display_set_osc(struct kobject *kobj,
	...
	prj_id = get_project();
	ret = kstrtouint(buf, 10, &temp_save);
	printk(KERN_INFO "prj_id is %d, osc mode = %d\n", prj_id, temp_save);
	...
	if (prj_id  == 24751 || prj_id == 24752 || prj_id == 24782) {
		mtk_crtc_osc_freq_switch(crtc, temp_save, 0);
		printk(KERN_ERR "osc switch\n");
	}
	osc_mode = temp_save;
...
void oplus_sync_panel_brightness(struct drm_crtc *crtc)
{
	...
	if ((get_project() == 21135) || (get_project() == 22023) || (get_project() == 22223)) {
		mtk_drm_setbacklight_without_lock_v0(crtc, mtk_crtc->oplus_pending_backlight, 0, 0x1 << SET_BACKLIGHT_LEVEL);
	} else {

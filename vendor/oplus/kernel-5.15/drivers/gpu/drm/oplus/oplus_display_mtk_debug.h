...
enum oplus_disp_drv_log_level {
	...
	OPLUS_DISP_DRV_LOG_LEVEL_BACKLIGHT = 4,
};
...
#define DISP_BACKLIGHT(fmt, arg...)	\
	do {	\
		if ((oplus_disp_drv_log_level >= OPLUS_DISP_DRV_LOG_LEVEL_BACKLIGHT) || (g_mobile_log)) \
			pr_info("[DISP_DRV][%s:%d]"pr_fmt(fmt), __func__, __LINE__, ##arg);	\
	} while (0)

void oplus_display_get_panel_brightness_time(void);

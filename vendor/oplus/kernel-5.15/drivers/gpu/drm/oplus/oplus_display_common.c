...
extern char display_brightness_time[32];
char brightness_time[32] = {0};
...
unsigned int oplus_display_panel_max_brightness = 4094;
...
int oplus_display_get_brightness_time(void *data) {
	...
	crtc = list_first_entry(&(ddev)->mode_config.crtc_list,
			typeof(*crtc), head);
	if (!crtc) {
		pr_err("find crtc fail, p_time=%p\n", p_time);
		return -EINVAL;
	}

	mtk_crtc = to_mtk_crtc(crtc);
	if (!mtk_crtc || !mtk_crtc->panel_ext || !mtk_crtc->panel_ext->params) {
		pr_err("falied to get crtc info\n");
		return -EINVAL;
	}
	if (!strcmp(display_brightness_time, "")) {
		memcpy(p_time->brightness_time, brightness_time,
				sizeof(brightness_time) >= 31?31:(sizeof(brightness_time)+1));
	} else {
		memcpy(p_time->brightness_time, display_brightness_time,
				sizeof(display_brightness_time) >= 31?31:(sizeof(display_brightness_time)+1));
	}
	pr_info("get brightness time: brightness_time[%s]\n", p_time->brightness_time);
	return 0;
}

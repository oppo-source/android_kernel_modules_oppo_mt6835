...
static int sgm7220_initialization(struct tcpc_device *tcpc)
{
	...
	/* CURRENT MODE ADVERTISE 1.5A [bit 7:6] */
...
static int sgm7220_set_duty(struct tcpc_device *tcpc, enum drp_toggle_type val)
{
	struct sgm7220_chip *chip = tcpc_get_dev_data(tcpc);
	int ret;
	if(val < TOGGLE_DFP_DRP_30 || val > TOGGLE_DFP_DRP_60) {
		return -EINVAL;
	}
	pr_debug("%s: update reg DRP_DUTY_CYCLE,val = %d!\n", __func__, val);
	ret = sgm7220_update_reg(chip->client, REG_INT,
							 val << INT_DRP_DUTY_CYCLE_SHIFT, INT_DRP_DUTY_CYCLE);
	if (ret < 0) {
		pr_err("%s: update reg fail!\n", __func__);
...
static int sgm7220_tcpcdev_init(struct sgm7220_chip *chip, struct device *dev)
{
	...
	*/
	...
	if (of_property_read_u32(np, "sgm7220,drp_toggle_type", &val) >= 0) {
		if(val > TOGGLE_DFP_DRP_60)
			chip->duty = TOGGLE_DFP_DRP_30;
		else
			chip->duty = val;
	} else {
		dev_info(dev, "use default duty\n");
		chip->duty = TOGGLE_DFP_DRP_30;
	}
...
#else
static int sgm7220_i2c_probe(struct i2c_client *client,const struct i2c_device_id *id)
#endif
{
	struct sgm7220_chip *chip;
	int ret = 0, chip_id;
	...
	sgm7220_set_duty(chip->tcpc, chip->duty);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0))
static void sgm7220_i2c_remove(struct i2c_client *client)
{
	struct sgm7220_chip *chip = i2c_get_clientdata(client);

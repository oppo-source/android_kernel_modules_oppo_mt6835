...
lcf_strategy_alloc_by_node(struct device_node *node)
{
	...
	for (i = 0; i < STRATEGY_TEMP_RANGE_MAX; i++) {
		...
		chg_info("lcf->curves_temp_range[%d].curves:\n", i);
		for (j = 0; j < lcf->curves_temp_range[i].num; j++) {
			chg_info("%d ", lcf->curves_temp_range[i].curves[j].iterm);
			chg_info("%d ", lcf->curves_temp_range[i].curves[j].vterm);
			chg_info("%d ", lcf->curves_temp_range[i].curves[j].exit);
		}
	}

...
static void oplus_pps_flash_mode_handle(struct oplus_pps *chip)
{
	...
	if (!chip || !chip->comm_topic || !chip->cpa_topic || !chip->pps_not_allow_votable) {
		chg_err("invalid chip or comm_topic or cpa_topic or pps_not_allow_votable\n");
		return;
	}
	...
	if (rc < 0) {
		...
		flash_on = !!data.intval;
	}
	rc = oplus_mms_get_item_data(chip->cpa_topic, CPA_ITEM_ALLOW, &data, false);
	if (rc < 0)
		return;
	if (flash_on && data.intval == CHG_PROTOCOL_PPS && chip->pps_online) {

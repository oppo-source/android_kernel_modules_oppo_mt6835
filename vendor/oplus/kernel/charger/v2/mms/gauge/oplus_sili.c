...
void oplus_gauge_set_last_cc(struct oplus_mms *mms, int cc)
{
	int rc = 0;
	struct oplus_mms_gauge *chip;
	int i;
...
void oplus_gauge_get_ddrc_status(struct oplus_mms *mms)
{
	...
	deep_info->index += snprintf(deep_info->msg, DEEP_INFO_LEN, "$$track_reason@@vote$$temp_p@@%d"
...
static int oplus_mms_gauge_upload_deep_dischg(struct oplus_mms_gauge *chip, char *deep_msg, bool main_batt)
{
	...
	if (main_batt) {
		...
		if (msg == NULL) {
			chg_err("alloc ERR_ITEM_DEEP_DISCHG_INFO error msg error\n");
			return -ENOMEM;
		}
	} else {
		msg = oplus_mms_alloc_str_msg(
			MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, ERR_ITEM_SUB_DEEP_DISCHG_INFO, deep_msg);
		if (msg == NULL) {
			chg_err("alloc ERR_ITEM_SUB_DEEP_DISCHG_INFO error msg error\n");
			return -ENOMEM;
		}
	}
...
void oplus_gauge_sub_deep_track_work(struct work_struct *work)
{
	...
	chip->sub_deep_info.index += snprintf(&(chip->sub_deep_info.msg[chip->sub_deep_info.index]),

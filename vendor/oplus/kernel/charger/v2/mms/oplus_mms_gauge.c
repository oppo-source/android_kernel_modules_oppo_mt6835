...
#define INVALID_MAX_VOLTAGE 4000
#define INVALID_MIN_VOLTAGE 2000
#define INVALID_CC_VALUE 2000
...
static void oplus_gauge_get_cc_limits(struct oplus_mms_gauge *chip)
{
	union mms_msg_data data = { 0 };
	int rc = 0, index = 0;

	if (!chip || !chip->deep_spec.support || chip->deep_spec.cc_curves.nums <= 0)
		return;

	rc = oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_CC, &data, true);
	if (rc != 0) {
		chg_err("get cc error %d\n", rc);
		return;
	}

	if (data.intval >= INVALID_CC_VALUE) {
		chg_err("cc %d invalid, cancel cc vote\n", data.intval);
		vote(chip->gauge_term_voltage_votable, CYCLE_COUNT_VOTER, false, 0, false);
		oplus_mms_gauge_update_super_endurance_mode_status_work(&chip->update_super_endurance_mode_status_work);
		vote(chip->gauge_shutdown_voltage_votable, CYCLE_COUNT_VOTER, false, 0, false);
		return;
	}

	for (index = chip->deep_spec.cc_curves.nums - 1; index >= 0; index--) {
		if (data.intval >= chip->deep_spec.cc_curves.limits[index].count) {
			vote(chip->gauge_term_voltage_votable, CYCLE_COUNT_VOTER, true,
			     chip->deep_spec.cc_curves.limits[index].vbat1, false);
			oplus_mms_gauge_update_super_endurance_mode_status_work(
				&chip->update_super_endurance_mode_status_work);
			vote(chip->gauge_shutdown_voltage_votable, CYCLE_COUNT_VOTER, true,
			     chip->deep_spec.cc_curves.limits[index].vbat0, false);
			break;
		}
	}
}
	...
	for (index = chip->deep_spec.count_curves.nums - 1; index >= 0; index--) {
		...
		if (chip->deep_spec.counts >= counts) {
			...
			vote(chip->gauge_term_voltage_votable, DEEP_COUNT_VOTER, true,
			     chip->deep_spec.count_curves.limits[index].vbat1, false);
			oplus_mms_gauge_update_super_endurance_mode_status_work(
				&chip->update_super_endurance_mode_status_work);
			vote(chip->gauge_shutdown_voltage_votable, DEEP_COUNT_VOTER, true,
			     chip->deep_spec.count_curves.limits[index].vbat0, false);
			break;
		}
	}
...
*/
	...
	if (chip->wired_online || chip->wls_online) {
		...
		oplus_gauge_get_cc_limits(chip);
	}
}
						  ...
						  bool step)
{
	...
	chg_info("term voltage vote client %s, volt = %d current = %d\n", client, volt, current_volt);

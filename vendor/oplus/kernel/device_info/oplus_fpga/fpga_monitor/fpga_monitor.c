...
#define SMEM_OPLUS_FPGA_PROP  124
...
static void fpga_heartbeat_work(struct work_struct *work)
{
	...
	mnt_pri->version_m = mnt_pri->version_s = buf[3];

	status->io_tx_err_cnt = buf[REG_IO_TX_ERR];
	status->io_rx_err_cnt = buf[REG_IO_RX_ERR];

	status->i2c_tx_err_cnt = buf[REG_I2C_TX_ERR];
	status->i2c_rx_err_cnt = buf[REG_I2C_RX_ERR];

	status->spi_tx_err_cnt = buf[REG_PRIMARY_SPI_ERR];
	status->spi_rx_err_cnt = buf[REG_SUB_SPI_ERR];

	status->gpio_status_err_cnt = buf[REG_GPIO_STATUS_ERR];
	status->i2c_status_err_cnt = buf[REG_I2C_STATUS_ERR];

	all_status->io_tx_err_cnt += status->io_tx_err_cnt;
	all_status->io_rx_err_cnt += status->io_rx_err_cnt;

	all_status->i2c_tx_err_cnt += status->i2c_tx_err_cnt;
	all_status->i2c_rx_err_cnt = status->i2c_rx_err_cnt;

	all_status->spi_tx_err_cnt += status->spi_tx_err_cnt;
	all_status->spi_rx_err_cnt += status->spi_rx_err_cnt;

	all_status->gpio_status_err_cnt += status->gpio_status_err_cnt;
	all_status->i2c_status_err_cnt += status->i2c_status_err_cnt;

	if (buf[REG_SLAVER_ERR] == 0xff) {
		status->slave_err_cnt++;
		fpga_rst_control(mnt_pri);
		fpga_exception_report(&mnt_pri->exception_data, EXCEP_SLAVER_ERR, SLAVER_ERROR, sizeof(SLAVER_ERROR));
	}

	if (status->io_tx_err_cnt || status->io_rx_err_cnt
	    || status->i2c_tx_err_cnt || status->i2c_rx_err_cnt
	    || status->spi_tx_err_cnt || status->spi_rx_err_cnt || buf[REG_SLAVER_ERR] == 0xff) {
		FPGA_INFO("fpga status error: (%llu %llu %llu %llu %llu %llu %llu %llu %llu)\n",
			  status->io_tx_err_cnt, status->io_rx_err_cnt, status->i2c_tx_err_cnt,
			  status->i2c_rx_err_cnt, status->spi_tx_err_cnt, status->spi_rx_err_cnt,
			  status->slave_err_cnt, status->gpio_status_err_cnt, status->i2c_status_err_cnt);
		fpga_kevent_fb(payload, 1024, FPGA_FB_STATUS_TYPE,
			       "NULL$$EventField@@fpgaStatusRead$$iotx@@%d$$iorx@@%d$$i2ctx@@%d$$i2crx@@%d$$spitx@@%d$$spirx@@%d$$slaveerr@@%d$$gpioerr@@%d$$i2cerr@@%d",
			       ret);
...
out:
	/*
	FPGA_INFO("master version [0x%x], year %x month %x day %x", buf[REG_MASTER_VERSION], buf[REG_MASTER_VER_YEAR], buf[REG_MASTER_VER_MON], buf[REG_MASTER_VER_DAY]);
	FPGA_INFO("slave version [0x%x], year %x month %x day %x", buf[REG_SLAVE_VERSION], buf[REG_SLAVE_VER_YEAR], buf[REG_SLAVE_VER_MON], buf[REG_SLAVE_VER_DAY]);
	*/
	FPGA_INFO("io_tx:%llu  io_rx:%llu  i2c_tx:%llu  i2c_rx:%llu  spi_tx:%llu  spi_rx:%llu", status->io_tx_err_cnt,
		  status->io_rx_err_cnt, status->i2c_tx_err_cnt, status->i2c_rx_err_cnt,
		  status->spi_tx_err_cnt, status->spi_rx_err_cnt);
	FPGA_INFO("slave status 0x%x, gpio status 0x%x, i2c status 0x%x",
		  buf[REG_SLAVER_ERR], buf[REG_GPIO_STATUS_ERR], buf[REG_I2C_STATUS_ERR]);
...
static int fpga_info_func(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "Device m version:\t\t%d\n", info->version_m);
	seq_printf(s, "Device s version:\t\t%d\n", info->version_s);

	seq_printf(s, "Device manufacture:\t\t%s\n", info->manufacture);

	seq_printf(s, "Device fw_path:\t\t%d\n", info->prj_id);
...
static int fpga_monitor_init(struct fpga_mnt_pri *mnt_pri)
{
	...
	mnt_pri->version_m = buf[3];
	mnt_pri->version_s = buf[3];
	return 0;
}
...
*/
	...
	queue_delayed_work(mnt_pri->hb_workqueue, &mnt_pri->hb_work, msecs_to_jiffies(5000));
...
static int fpga_monitor_resume_early(struct device *dev)
{
	...
	gpio_direction_output(pdata->sleep_en_gpio, 0);
	/* usleep_range(CLK_TO_SLEEP_CONTROL_TIME, CLK_TO_SLEEP_CONTROL_TIME); */
	gpio_direction_output(pdata->clk_switch_gpio, 0);
	return 0;
}

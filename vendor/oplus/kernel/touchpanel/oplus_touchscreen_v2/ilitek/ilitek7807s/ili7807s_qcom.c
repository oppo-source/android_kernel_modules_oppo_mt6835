...
void ili_report_ap_mode(u8 *buf, int len)
{
	...
	for (i = 0; i < MAX_TOUCH_NUM; i++) {
		...
		if (ilits->ts->snr_read_support && snr->doing) {
			...
			snr->channel_x = snr->x * 18 / ilits->ts->resolution_info.max_x;
			snr->channel_y = snr->y * 32 / ilits->ts->resolution_info.max_y;
			ILI_INFO("MTC_log: tx_num is %d rx_num is %d\n", ilits->hw_res->tx_num, ilits->hw_res->rx_num);
				  ...
				  u8 read_type)
{
	...
	if (i < 3) {
		...
		for (i = 0; i < MAX_TOUCH_NUM; i++) {
			if (ilits->rib.nReportResolutionMode == OFF) {
				if ((buf[(3 * i) + 5] != 0xFF) || (buf[(3 * i) + 6] != 0xFF)
					|| (buf[(3 * i) + 7] != 0xFF)) {
				break; // has touch
				}
			} else if (ilits->rib.nReportResolutionMode == ON) {
				if ((buf[(4 * i) + 5] != 0xFF) || (buf[(4 * i) + 6] != 0xFF)
					|| (buf[(4 * i) + 7] != 0xFF) || (buf[(4 * i) + 8] != 0xFF)) {
					break; // has touch
			}
			} else if (ilits->rib.nReportResolutionMode == POSITION_DIFFER_LOW_RESOLUTION) {
				if ((buf[(4 * i) + 5] != 0xFF) || (buf[(4 * i) + 6] != 0xFF)
					|| (buf[(4 * i) + 7] != 0xFF)) {
					break; // has touch
				}
			} else if (ilits->rib.nReportResolutionMode == POSITION_DIFFER_HIGH_RESOLUTION) {
				if ((buf[(5 * i) + 5] != 0xFF) || (buf[(5 * i) + 6] != 0xFF)
					|| (buf[(5 * i) + 7] != 0xFF) || (buf[(5 * i) + 8] != 0xFF)) {
					break; // has touch
				}
			} else {
				ILI_INFO("not susport ResolutionMode(0x%X)\n", ilits->rib.nReportResolutionMode);
			}
		}
...
START_TO_READ:
    if (is_delta_snr) {
        ...
        temp = (s16)((buf[(snr->channel_y * xch + snr->channel_x) * 2 + offset_len] << 8)
            + buf[(snr->channel_y * xch + snr->channel_x) * 2 + offset_len + 1]);
        if (index) {

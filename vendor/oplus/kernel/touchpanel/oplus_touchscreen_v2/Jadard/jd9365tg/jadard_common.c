...
uint16_t jd_gest_key_def[JD_GEST_SUP_NUM] = {
    1, 251, 252, 253, 254, 255, 256, 12,
    6, 259, 2, 3, 4, 5, 13, 265,
    266, 267, 268, 269, 7, 271, 272
};
...
static void jadard_wake_event_parse(struct jadard_ts_data *ts, int ts_status)
{
    ...
    for (i = 0; i < JD_GEST_SUP_NUM; i++) {
        if (wake_event == jd_gest_event[i]) {
            ...
            for (j = 0; j < JD_GEST_TRACK_NUM; j++) {
				pCoord_info = pjadard_report_data->touch_coord_info + (JD_FINGER_DATA_SIZE * (j + 2));
                if ((pCoord_info[0] == 0xFF) && (pCoord_info[1] == 0xFF)
                    && (pCoord_info[2] == 0xFF) && (pCoord_info[3] == 0xFF)) {
                    pjadard_host_data->gesture_track_data[j * 2]  = 0;
				    pjadard_host_data->gesture_track_data[j * 2 + 1]  = 0;
                } else {
                    pjadard_host_data->gesture_track_data[j * 2]  = (uint16_t)(pCoord_info[0] << 8 | pCoord_info[1]);
                    pjadard_host_data->gesture_track_data[j * 2 + 1]  = (uint16_t)(pCoord_info[2] << 8 | pCoord_info[3]);
                }

				JD_I("Gtrack id=%d, x=%d, y=%d\n", j, pjadard_host_data->gesture_track_data[j * 2], pjadard_host_data->gesture_track_data[j * 2 + 1]);
			}

            pCoord_info = pjadard_report_data->touch_coord_info + (JD_FINGER_DATA_SIZE * (JD_GEST_TRACK_NUM + 2));
            pjadard_host_data->gesture_clockwise = pCoord_info[0];
...
static int jadard_parse_report_points(struct jadard_ts_data *ts, int ts_status)
{
    ...
    pjadard_host_data->pointid_info = 0;
#ifdef JD_REPORT_FRAME_CHECK
    if (pjadard_host_data->pre_finger_frame != pjadard_host_data->finger_frame) {
#endif
        if (pjadard_ic_data->JD_X_RES > pjadard_ic_data->JD_Y_RES) {
        ...
        for (i = 0; i < pjadard_ic_data->JD_MAX_PT; i++) {
            ...
            if ((x >= 0 && x <= ts->pdata->abs_x_max) &&
                ...
                /* Set coordinate */
                pjadard_host_data->pointid_info |= (1 << i);
...
uint32_t jadard_ts_work(struct jadard_ts_data *ts, int gesture_enable, int is_suspended)
{
    ...
    case JD_REPORT_COORD:
        ...
        if (ts_status == JD_TS_UNUSUAL_DATA_FAIL) {
            oplus_status = IRQ_IGNORE;
        } else {
            oplus_status = IRQ_TOUCH;
        }
        break;
    case JD_REPORT_SMWP_EVENT:
        ts_status = jadard_ts_operation(ts, irq_event, ts_status);
    ...
    JD_D("%s: ts_status: %d, oplus_status: %d\n", __func__, ts_status, oplus_status);
    JD_D("%s: en_touch_event_helper: %d\n", __func__, ts->jadard_oplus_ts_backup->en_touch_event_helper);
    JD_D("%s: enable_point_auto_change: %d\n", __func__, ts->jadard_oplus_ts_backup->enable_point_auto_change);
    /* return status for oplus */
    return oplus_status;
}
                                    ...
                                    size_t len, loff_t *pos)
{
    ...
    if (!pjadard_debug->proc_send_flag) {
        ...
        if (buf_tmp != NULL) {
            g_module_fp.fp_sorting_test(buf_tmp, len, NULL, NULL);
            ret = strlen(buf_tmp);
...
//#endif
    ...
    if (err) {
        JD_E("%s: Unable to register input device\n", __func__);
    ...
    /* Initial for oplus common start */
    ts->module_fp = &g_module_fp;
    ts->ic_data = pjadard_ic_data;
    ts->int_enable= jadard_int_enable;
    ts->pjadard_ts_work = jadard_ts_work;
    ts->host_data = pjadard_host_data;
    /* Initial for oplus common end */
...
int jadard_oplus_chip_common_suspend(struct touchpanel_data *oplus_ts)
{
    JD_I("%s: tp_pm_suspend\n", __func__);
    tp_pm_suspend(oplus_ts);
    JD_I("%s: end\n", __func__);
    return 0;
}
...
int jadard_chip_common_resume(struct jadard_ts_data *ts)
{
    ...
    } else if (g_module_fp.fp_0f_upgrade_fw(jd_i_CTPM_firmware_name, NULL) < 0) {
...
int jadard_oplus_chip_common_resume(struct touchpanel_data *oplus_ts)
{
    JD_I("%s: tp_pm_resume\n", __func__);
    tp_pm_resume(oplus_ts);
    JD_I("%s: end \n", __func__);
    return 0;
}

...
#define JADARD_DRIVER_VER        "01.C5" /* MAJOR_VER.MINOR_VER */
#define JADARD_DRIVER_CID_VER    "00.05" /* MAJOR_CID_VER.MINOR_CID_VER */
...
struct jadard_ts_data {
    /* Set data and function for oplus common start */
    struct jadard_module_fp *module_fp;
    struct jadard_ic_data *ic_data;
    struct jadard_report_data *report_data;
    void (*int_enable)(bool enable);
    struct hw_resource *hw_res;
    struct touchpanel_data *jadard_oplus_ts_backup;
    struct firmware *p_firmware_headfile;
    uint32_t (*pjadard_ts_work)(struct jadard_ts_data *ts, int gesture_enable, int is_suspended);
    struct jadard_host_data *host_data;
    tp_dev tp_type;
    uint8_t touch_direction;
    struct firmware tp_fw;
	int tp_index;
    /* Set data and function for oplus common end */
    bool suspended;
...
struct jadard_host_data {
    ...
    /* oplus gesture info. */
    int gesture_track_data[12];
    uint32_t gesture_clockwise;
};
...
extern int jadard_oplus_chip_common_suspend(struct touchpanel_data *oplus_ts);
extern int jadard_oplus_chip_common_resume(struct touchpanel_data *oplus_ts);

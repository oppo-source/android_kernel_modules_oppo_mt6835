...
void operate_mode_switch(struct touchpanel_data *ts)
{
	...
	if (ts->is_suspended) {
		...
		if (ts->disable_touch_event_support) {
			mode_switch_health(ts, MODE_UNDERWATER, ts->touch_event_diasble);
			TP_INFO(ts->tp_index, "ODE_UNDERWATER success: touch_event_diasble = %d\n", __func__, ts->touch_event_diasble);
		}
